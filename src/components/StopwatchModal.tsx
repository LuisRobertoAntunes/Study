    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex justify-center items-center z-50 p-4">
      ...
      <div className="w-full max-w-4xl p-8 rounded-2xl text-gray-900 dark:text-white flex flex-col bg-white dark:bg-gray-800">
          ...
          <div className="flex items-center space-x-4 flex-shrink-0">
            <div className="w-48">
              <select
                ...
                className="w-full rounded-md border border-gray-300 bg-gray-50 px-3 py-2 text-gray-900 focus:outline-none focus:ring-2 focus:ring-gold-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
              >
            ...
            <div className="w-48">
              <select
                ...
                className="w-full rounded-md border border-gray-300 bg-gray-50 px-3 py-2 text-gray-900 focus:outline-none focus:ring-2 focus:ring-gold-500 disabled:opacity-50 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
              >
        ...
        <div className="w-full bg-gray-800/50 rounded-full h-8 mb-4 shadow-inner overflow-hidden dark:bg-gray-700/50">
          {mode === 'timer' ? (
            <div
              className="bg-gradient-to-r from-gold-400 to-gold-500 h-8 rounded-full transition-all duration-500 ease-out shadow-lg shadow-gold-500/30"
        ...
        <div className="flex items-center justify-center mb-8">
          {(!isRunning && mode === 'timer') ? (
            <input
              ...
              className="text-8xl font-mono font-bold text-center tracking-wider bg-transparent border-none focus:outline-none focus:ring-0 w-full text-gold-500 dark:text-gold-300"
            />
          ) : (
            <div className="text-8xl font-mono font-bold text-center tracking-wider text-gold-500 dark:text-gold-300 w-full">
              {formatTime(time)}
            </div>
          )}
          <div className="flex items-center ml-4">
            <div className="flex flex-col">
              <button onClick={() => setMode('cronometro')} className={`px-3 py-1 text-sm font-semibold rounded-full ${mode === 'cronometro' ? 'bg-gold-600 text-gray-900 dark:text-white dark:bg-gold-700' : 'text-gray-400 dark:text-gray-500'}`}>CRONÔMETRO</button>
              <button onClick={() => setMode('timer')} className={`mt-2 px-3 py-1 text-sm font-semibold rounded-full ${mode === 'timer' ? 'bg-gold-600 text-gray-900 dark:text-white dark:bg-gold-700' : 'text-gray-400 dark:text-gray-500'}`}>TIMER</button>
            </div>
            <button
              onClick={handleGetRecommendation}
              title="Sugerir Próximo Estudo"
              className="ml-4 p-3 bg-gold-500 hover:bg-gold-600 text-white rounded-full shadow-lg shadow-gold-500/50 transition-colors"
            >
        ...
        <div className="flex justify-center space-x-6">
          {!isRunning ? (
            <button onClick={handlePlay} className="w-20 h-20 bg-gold-500 rounded-full flex items-center justify-center text-white shadow-lg hover:bg-gold-600 transition-colors dark:bg-gold-600 dark:hover:bg-gold-700">
            ...
            <button onClick={handlePause} className="w-20 h-20 bg-gold-500 rounded-full flex items-center justify-center text-white shadow-lg hover:bg-gold-600 transition-colors dark:bg-gold-600 dark:hover:bg-gold-700">
          ...
          {hasStarted && (
            <button onClick={handleReset} className="w-20 h-20 bg-gold-500 rounded-full flex items-center justify-center text-white shadow-lg hover:bg-gold-600 transition-colors dark:bg-gold-600 dark:hover:bg-gold-700">

  ...
  return (
    <div className={`relative ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`} ref={dropdownRef}>
      <div
        ...
        className="mt-1 flex items-center w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm px-3 focus:outline-none focus:ring-gold-500 focus:border-gold-500 bg-white text-gray-900 dark:bg-gray-700 dark:text-gray-100 sm:text-sm cursor-pointer h-11 overflow-hidden"
      >
        <div className="flex flex-wrap gap-2">
          {selectedOptions.length === 0 ? (
            ...
            selectedOptions.map(option => (
              <span
                key={option}
                className="flex items-center gap-1 bg-gold-100 dark:bg-gold-800 text-gold-800 dark:text-gold-100 text-xs font-semibold px-2 py-1 rounded-full truncate"
              >
            ...
            {options?.map(option => (
              ...
              >
                <input
                  ...
                  readOnly
                  className="mr-2 form-checkbox h-4 w-4 text-gold-600 border-gray-300 dark:border-gray-500 rounded focus:ring-gold-500"
                />

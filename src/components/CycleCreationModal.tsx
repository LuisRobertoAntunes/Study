        ...
        return (
          <div className="text-center bg-white dark:bg-gray-800 p-8 rounded-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div onClick={() => setModalStep(4)} className="group border-2 border-gray-200 hover:border-gold-500 p-6 rounded-lg cursor-pointer transition-all duration-300 transform hover:scale-105 dark:border-gray-700 dark:hover:border-gold-400">
                <FaTools className="text-4xl text-gold-500 mx-auto mb-4 transition-transform duration-300 group-hover:rotate-12 dark:text-gold-400" />
        ...
        return (
          <div className="bg-white dark:bg-gray-800 p-8 rounded-lg">
                ...
                <div className="space-y-2 overflow-y-auto pr-2" style={{ maxHeight: '260px' }}>
                  {filteredSubjects.map(s => (
                    <div
                      ...
                      className={`p-3 rounded-lg cursor-pointer transition-all duration-200 flex justify-between items-center ${selectedSubjectIds.includes(s.id) ? 'bg-gold-100 text-gold-800 font-semibold dark:bg-gold-800 dark:text-gold-100' : 'bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-gray-100'}`}>
                      <span className="text-gray-800">{s.subject}</span>
                      {selectedSubjectIds.includes(s.id) && <FaCheckCircle className="text-gold-500 dark:text-gold-400" />}
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="flex justify-between mt-8">
              ...
              <button onClick={() => setModalStep(2)} className="px-6 py-2 bg-gold-500 text-white font-semibold rounded-lg hover:bg-gold-600 transition-colors dark:bg-gold-600 dark:hover:bg-gold-700" disabled={selectedSubjectIds.length === 0}>Avançar</button>
            </div>
          </div>
        );
      case 2:
        ...
        const StarRating = ({ value, onChange }: { value: number, onChange: (value: number) => void }) => (
          <div className="flex space-x-1">
            {[1, 2, 3, 4, 5].map(star => (
              <FaStar
                key={star}
                className={`cursor-pointer ${star <= value ? 'text-gold-400' : 'text-gray-300'}`}
      ...
      case 3:
        return (
          <div className="bg-white dark:bg-gray-800 p-8 rounded-lg">
            ...
            <div className="p-6 bg-gold-50 dark:bg-gold-900/50 rounded-lg border-2 border-gold-200 dark:border-gold-700">
